...
$(document).ready(function () {
  ...
  a.on("click", function () {
    ...
    $(document).mouseup(function (e) { // событие клика по веб-документу
      var div = $('.flabFaq__acardionItem span, .flabFaq__desc'); // тут указываем ID элемента
      if (!div.is(e.target) // если клик был не по нашему блоку
        &&
        div.has(e.target).length === 0) { // и не по его дочерним элементам
        $('.flabFaq__desc').slideUp(); // скрываем его
        $('.flabFaq__acardionItem').removeClass('accItem'); // скрываем его
        $('.flabFaq__acardionItem i').removeClass('positionIcon'); // скрываем его
      }
    });
  ...
  }).init(), $(".mmenu-js ul li a").click(function (e) {
    e.preventDefault();
